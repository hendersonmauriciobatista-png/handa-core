...
class MarketRadarEngine:
    ...
    def _is_symbol_blocked(self, symbol: str):
        ...
        if self.block_last_traded_symbol and self.last_traded_symbol == symbol:
            return True
    ...
    def _passes_quality_filter(self, item):
        ...
        if raw_score < self.min_final_score:
    ...
    def _filter_ranking(self, ranking):
        ...
        for item in ranking:
            ...
            # ===== MCE TEMPORARIAMENTE DESATIVADO =====
    ...
    def _get_dynamic_min_final_score(self) -> float:
        ...
        # Mercado forte: mantém filtro rígido
        ...
        if refined_count > 0 and approved_count == 0:
            dynamic_score = min(dynamic_score, 0.26)
        ...
        if mqii_score <= 0.10:
            dynamic_score = max(dynamic_score, 0.30)
