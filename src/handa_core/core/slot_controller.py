...
class SlotController:
    ...
    def _register_loss_cooldown(self, symbol: str):
        ...
        self._block_rejected_symbol(symbol, cycles=rejection_cycles)

        print(
            f"[COOLDOWN] {symbol} BLOQUEADO | "
            f"loss_streak={streak} | "
            f"cooldown={cooldown_seconds}s | "
            f"rejection_cycles={rejection_cycles}"
        )
    ...
    def _pick_opportunity_for_slot(
        ...
        for opportunity in opportunities:
            ...
            if self._is_rejected_symbol_blocked(symbol):
                print(f"[REJECTION COOLDOWN] {symbol} ainda bloqueado entre ciclos")
                attempted_symbols.add(symbol)
                continue
                ...
                try:
                    if signal:
                        ...
                        if confidence == "MUITO_BAIXA":
                            ...
                            # 🔥 NOVO: cooldown mais forte para ALO
    ...
    def _execute_buy(self, slot):
        ...
        try:
            ...
            if self._is_pair_blocked(slot.pair):
                print(f"[SLOT {slot.slot_id}] BUY BLOQUEADO POR COOLDOWN: {slot.pair}")
            ...
            if self._is_rejected_symbol_blocked(slot.pair):
                print(
                    f"[SLOT {slot.slot_id}] BUY BLOQUEADO POR REJECTION COOLDOWN: {slot.pair}"
                )
                slot.pending_buy_signal = None
                slot.reset()
                return
