...
class MarketQualityEvaluator:
    ...
    def evaluate(self, agg: MarketQualityAggregate) -> MarketQualitySnapshot:
        ...
        raw_momentum = (
            (agg.bullish_momentum_count * 1.0)
            + (agg.neutral_momentum_count * 0.8)
        )

        momentum_component = self._clamp(raw_momentum / total)

        volume_component = self._clamp(agg.avg_volume_ratio / 1.6)

        approval_component = self._safe_div(agg.refined_count, total)
