...
class DecisionEngine:
    ...
    def evaluate_buy(self, snapshot: MarketSnapshot) -> Optional[BuySignal]:
        ...
        # ==========================================================
        # 🔥 MQII + MARKET PROTECTION (CAUTIOUS FILTER)
        # ==========================================================
        try:
            ...
            if mqii:
                ...
                market_state = str(snapshot.market_state).strip().upper()
                momentum = str(snapshot.momentum).strip().upper()

                if state == "NO_TRADE":
                    logger.info(f"[Engine] BLOQUEADO POR MQII NO_TRADE: {pair}")
                    return None
                ...
                if (
                    ...
                    logger.info(
                        f"[Engine] BLOQUEADO POR MQII CAUTIOUS + MARKET FRACO: {pair}"
                    )
                    return None

        except Exception as e:
            logger.warning(f"[Engine MQII FILTER ERROR] {e}")
        ...
        if penalty >= 2:
            logger.info(f"[Engine] BLOQUEADO POR PENALTY: {pair} | penalty={penalty}")
            return None
        ...
        spread = abs(ema_fast - ema_slow) / price if price > 0 else 0.0

        if spread < 0.0003:
            logger.info(
                f"[Engine] BLOQUEADO LATERALIZAÇÃO: {pair} | spread={spread:.6f}"
            )
            return None
        ...
        if not approved:
            ...
            if qualifier_result.approved and qualifier_result.mode == "OPPORTUNITY":
